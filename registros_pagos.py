...
# - Valor pago ENTERO (sin decimales) + visual 1.000.000
...
SHEET_COLUMNS = [
    ...
    "OBSERVACIONES",  # aquí medio de pago
    "CONCILIACIÓN",
    "OBSERVACIÓN",
    "ITEM",
...
def append_row_to_sheet(registro: dict):
    ...
    # ✅ La forma correcta: valueInputOption e insertDataOption van en query string
    url = (
    ...
    fila = [[registro.get(col, "") for col in SHEET_COLUMNS]]
    body = {"values": fila}

    resp = requests.post(url, headers=headers, json=body, timeout=15)

    if not (200 <= resp.status_code < 300):
        raise RuntimeError(f"Sheets HTTP {resp.status_code}: {resp.text}")
...
def format_miles_punto(n: int) -> str:
    # 1000000 -> "1.000.000"
    return f"{n:,}".replace(",", ".")
...
try:
    ...
    df_medios = normaliza(leer_excel_local(PATH_MEDIOS))
...
# Medios de pago: si no encuentra 'MEDIO', usa primera columna
...
if not col_medio_pago:
    st.error("❌ La base Medios_de_pago.xlsx no tiene columnas.")
    st.stop()
...
# ✅ Visual bonito: 1.000.000
st.caption(f"Valor ingresado: $ {format_miles_punto(int(valor_pago))}")
...
medio_pago_sel = st.selectbox("💳 Medio de pago:", lista_medios)
...
if st.button("✅ Registrar pago"):
    ...
    # Duplicados en CSV local (misma regla que ya tenías)
    ...
    # ✅ Guardamos el valor como string entero (no formateado) para no dañar Sheets
    valor_entero_str = str(int(valor_pago))
    ...
    registro = {
        ...
        "VALOR PAGO TOTAL": valor_entero_str,
        "VALOR PAGO POR CAMPAÑA": valor_entero_str,
        ...
        "OBSERVACIONES": medio_pago_sel,  # ✅ aquí queda el medio de pago
        "CONCILIACIÓN": "",
        "OBSERVACIÓN": "",
        "ITEM": "",
        "CONTACTO COLLECTIONS": "",
    ...
    try:
        ...
        st.info("📌 Valor guardado sin decimales y medio de pago guardado en OBSERVACIONES.")
    except Exception as e:
